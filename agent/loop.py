"""AgentLoop: Vòng lặp cốt lõi ReAct.

Quản lý ngữ cảnh 5 tầng:
  Tầng 1 (microcompact)     — cắt tỉa các kết quả công cụ cũ khi gặp áp lực bộ nhớ
  Tầng 2 (context_collapse) — thu gọn các khối văn bản dài mà không cần gọi LLM (chi phí bằng 0)
  Tầng 3 (auto_compact)     — tóm tắt có cấu trúc bằng LLM với bảo vệ giới hạn token phần đuôi
  Tầng 4 (compact tool)     — mô hình gọi trực tiếp công cụ compact để kích hoạt Tầng 3
  Tầng 5 (iterative update) — đợt nén thứ N cập nhật bản tóm tắt trước đó thay vì bắt đầu lại từ đầu

Thực thi công cụ (Tool execution):
  - Phân nhóm Đọc/Ghi: các công cụ chỉ đọc liên tiếp chạy song song qua các luồng (threads)
"""
...
from pseud.agent.grounding import GroundingLedger
from pseud.agent.memory import WorkspaceMemory
from pseud.agent.progress import HeartbeatTimer, ProgressEvent, _set_emitter
from pseud.agent.tools import ToolRegistry
from pseud.agent.trace import TraceWriter
from pseud.core.state import RunStateStore
from pseud.goal.context import (
    format_goal_continuation_prompt,
    get_current_goal_context,
    goal_needs_continuation,
    goal_progress_tuple,
)
from pseud.providers.chat import ChatLLM, LLMRuntimeSnapshot, ProviderStreamError
...
from pseud.config.paths import get_runs_dir, get_sessions_dir
from pseud.tools.redaction import redact_payload, redact_tool_result
...
def _override(name: str):
    """Trả về ghi đè cấp module (monkeypatched) nếu có."""
...
def _tool_timeout_seconds() -> float:
    ...
    if ov is not None:
        return ov
    from pseud.config.accessor import get_env_config
    return get_env_config().agent_tuning.kairos_tool_timeout_seconds
...
def _goal_max_continuations() -> int:
    ...
    if ov is not None:
        return ov
    from pseud.config.accessor import get_env_config
    return get_env_config().agent_tuning.kairos_goal_max_continuations

logger = logging.getLogger(__name__)


def _coerce_usage_int(value: Any) -> int:
    """Ép kiểu số lượng token từ nhà cung cấp thành số nguyên không âm."""
    try:
...
def _normalize_llm_usage(usage: Any) -> dict[str, int] | None:
    """Chuẩn hóa siêu dữ liệu sử dụng do nhà cung cấp báo cáo mà không ước tính token."""
    if usage is None:
        return None
...
def _new_llm_usage_summary(llm: Any) -> dict[str, Any]:
    """Tạo bộ tích lũy sử dụng của nhà cung cấp theo phạm vi lượt chạy."""
    from pseud.config.accessor import get_env_config
    cfg = get_env_config()
...
def _record_llm_usage(
    ...
    """Tích lũy và lưu trữ một sự kiện sử dụng do nhà cung cấp báo cáo."""
...
def _format_timeout(seconds: float) -> str:
    """Trả về nhãn thời gian chờ (timeout) dễ đọc cho con người."""
    if seconds < 1:
...
def estimate_tokens(messages: list) -> int:
    """Ước tính số lượng token xấp xỉ (~4 ký tự/token).
...
def _microcompact(messages: list) -> None:
    """Tầng 1: Tắt âm thầm các kết quả công cụ cũ, giữ lại N kết quả gần nhất.

    Args:
        messages: Danh sách tin nhắn (bị thay đổi trực tiếp tại chỗ).
    """
...
def _compact_messages_for_emergency(messages: list[dict[str, Any]]) -> None:
    """Thu gọn system_msg và user_msg khi payload bị HTTP 413 ngay ở iteration 1."""
    if not messages:
        return
    sys_content = messages[0].get("content")
    if isinstance(sys_content, str) and len(sys_content) > 3000:
        messages[0]["content"] = sys_content[:3000] + "\n[System prompt truncated for payload limit]"
    if len(messages) > 1:
        u_content = messages[1].get("content")
        if isinstance(u_content, str) and len(u_content) > 1500:
            messages[1]["content"] = u_content[:1500] + "\n[User message truncated for payload limit]"


def _compact_tool_definitions(tool_defs: list[dict[str, Any]], max_tools: int = 4) -> list[dict[str, Any]]:
    """Cắt bớt phần mô tả dài và loại bỏ tool dư thừa trong schema để vượt qua giới hạn HTTP 413 / TPM limit."""
    PRIMARY_CORE_TOOLS = [
        "scan_lakehouse_anomalies",
        "check_cemetery_guard",
        "synthesize_hypothesis",
        "dispatch_to_l25",
        "evidence_ledger",
        "quet_lat_cat_thay_the",
        "quet_do_nhay_tham_so",
    ]
    compacted = []
    included_count = 0
    for td in tool_defs:
        fn = td.get("function", {})
        name = fn.get("name", "")
        if name and name not in PRIMARY_CORE_TOOLS and included_count >= max_tools:
            continue
        ...
        if isinstance(params, dict) and "properties" in params:
            for k, v in params["properties"].items():
                if isinstance(v, dict):
                    prop_copy = dict(v)
                    if "description" in prop_copy and isinstance(prop_copy["description"], str):
                        prop_copy["description"] = prop_copy["description"][:40]
                    new_props[k] = prop_copy
                else:
                    new_props[k] = v
            params = {**params, "properties": new_props}
        ...
        included_count += 1
        if included_count >= max_tools and len(tool_defs) > max_tools:
            break
    return compacted


def _context_collapse(messages: list) -> None:
    """Tầng 2: Thu gọn các khối văn bản dài trong tin nhắn cũ mà không cần gọi LLM.

    Bảo tồn phần đầu + phần cuối của văn bản lớn, thu gọn phần giữa.
    Chi phí API bằng 0 — thao tác xử lý chuỗi thuần túy.
...
def _fix_tool_pairs(messages: list) -> None:
    """Sửa chữa các cặp tool_call / tool_result mồ côi sau khi nén.

    Hai việc sửa chữa:
      1. Loại bỏ các kết quả tool mà tool_call tương ứng đã bị nén mất.
      2. Chèn các kết quả giả (stub) cho các tool_call mà kết quả của chúng đã bị nén mất.

    Args:
        messages: Danh sách tin nhắn (bị thay đổi trực tiếp tại chỗ).
    """
    # Thu thập tất cả ID tool_call từ các tin nhắn của assistant
    ...
    # Loại bỏ các kết quả tool mồ côi
    i = 0
    ...
    # Thu thập các ID kết quả hiện có
    ...
    # Chèn kết quả stub cho các tool_call mồ côi
...
def _attach_tool_call_thought_signatures(message: dict[str, Any], tool_calls: list) -> dict[str, Any]:
    """Gắn chữ ký suy nghĩ (thought signature) của Gemini vào các lệnh gọi tool phát lại của assistant.

    Tin nhắn phát lại sau đó được chuyển đổi ngược lại thành các tin nhắn LangChain từ một
    lịch sử dict thuần túy. Giữ chữ ký ở cả khe trung lập nhà cung cấp
    ``extra_content.thought_signature`` và khe tương thích OpenAI của Gemini
    ``extra_content.google.thought_signature`` để cả các bài kiểm thử phát lại cục bộ
    và bộ tiêm yêu cầu Gemini đều có thể khôi phục giá trị.
    """
...
def _is_tool_success(result: str) -> bool:
    """Trả về True nếu kết quả tool không có dạng phản hồi lỗi."""
    try:
...
def _normalize_tool_run_dir(args: dict[str, Any], memory_run_dir: str | None) -> dict[str, Any]:
    """Chuẩn hóa ``run_dir`` trong đối số công cụ thành đường dẫn tuyệt đối khi có thể.

    Nếu mô hình cung cấp một ``run_dir`` tương đối (ví dụ ``"."`` hoặc
    ``"risk_parity_run"``), phân giải nó theo thư mục lượt chạy đang hoạt động.
    """
...
class AgentLoop:
    """Vòng lặp cốt lõi ReAct Agent.

    Thuộc tính:
        registry: Registry công cụ.
        llm: Client ChatLLM.
        memory: Bộ nhớ workspace.
    ...
    def __init__(
        ...
        Args:
            registry: Registry công cụ.
            llm: Client ChatLLM.
            memory: Bộ nhớ workspace (tạo mới nếu không cung cấp).
            event_callback: Callback sự kiện (event_type, data).
            max_iterations: Số vòng lặp tối đa.
            persistent_memory: PersistentMemory dùng cho ghi nhớ liên phiên.
        """
    ...
    def cancel(self) -> None:
        ...
        Đặt cờ an toàn luồng được kiểm tra ở mỗi biên vòng lặp, mỗi chunk
        luồng LLM, và giữa các đợt công cụ, để lượt chạy dừng lại tại
        điểm kiểm tra tiếp theo thay vì chỉ dừng ở vòng lặp kế tiếp.
        """
    ...
    def run(self, user_message: str, history: Optional[List[Dict[str, Any]]] = None, session_id: str = "") -> Dict[str, Any]:
        ...
        Args:
            ...
            history: Các tin nhắn trò chuyện trước đó.
        ...
        try:
            while iteration < self.max_iterations:
                ...
                try:
                    ...
                    # Emergency context compaction if HTTP 413, TPM, or payload limit occurred
                    ...
                    if (
                        ...
                        logger.warning(
                            "Payload/TPM limit hit (iter %s), performing emergency context compaction before retry",
                            current_iter,
                        )
                        _microcompact(messages)
                        _context_collapse(messages)
                        ...
                        _compact_messages_for_emergency(messages)
                        if tool_defs:
                            tool_defs = _compact_tool_definitions(tool_defs, max_tools=4)
                    ...
                    if token_delta or turn_delta:
                        try:
                            if goal_store is None:
                                from pseud.goal import GoalStore
                ...
                if not response.has_tool_calls:
                    ...
                    # Compute tools_executed BEFORE empty-content check so we
                    # can nudge instead of breaking when the pipeline is in-flight.
                    tools_executed = set()
                    for m in messages:
                        if m.get("role") == "tool" and m.get("name"):
                            tools_executed.add(m["name"])
                        elif m.get("role") == "assistant" and isinstance(m.get("tool_calls"), list):
                            for tc in m["tool_calls"]:
                                if isinstance(tc, dict):
                                    func = tc.get("function")
                                    if isinstance(func, dict) and func.get("name"):
                                        tools_executed.add(func["name"])
                                    elif tc.get("name"):
                                        tools_executed.add(tc["name"])

                    has_scanned = "scan_lakehouse_anomalies" in tools_executed
                    has_synthesized = "synthesize_hypothesis" in tools_executed or "dispatch_to_l25" in tools_executed

                    if not final_content:
                        # Pipeline in-flight (scanned but not synthesized):
                        # nudge the model instead of breaking on empty response.
                        if has_scanned and not has_synthesized and iteration < self.max_iterations and preamble_nudge_count < 3:
                            preamble_nudge_count += 1
                            logger.info(
                                "Empty model response but pipeline in-flight (nudge %s/3). Forcing next tool call.",
                                preamble_nudge_count,
                            )
                            messages.append({
                                "role": "system",
                                "content": (
                                    "[SYSTEM NOTE] Your response was empty. The lakehouse scan has completed "
                                    "and anomaly signals are available. You MUST now call `synthesize_hypothesis` "
                                    "to create Alpha Specs from the scan results, then `dispatch_to_l25` to "
                                    "send them to Layer 2.5. DO NOT output text — call the tool directly."
                                ),
                            })
                            continue
                    ...
                    # Check if model emitted preamble text without a tool call
                    # while research pipeline is in-flight.
                    if has_scanned and not has_synthesized:
                        if iteration < self.max_iterations and preamble_nudge_count < 3:
                            preamble_nudge_count += 1
                            logger.info("Model emitted preamble text without tool call (nudge %s/3). Directing model to next pipeline step.", preamble_nudge_count)
                            messages.append({"role": "assistant", "content": final_content})
                            messages.append({
                                "role": "system",
                                "content": (
                                    "[SYSTEM NOTE] You described your intention in text, but did not execute a tool call. "
                                    "DO NOT just output text. You MUST call the next tool in the pipeline now "
                                    "(such as `synthesize_hypothesis` to create Alpha Specs, `check_cemetery_guard`, or `dispatch_to_l25` to send to Layer 2.5)."
                                ),
                            })
                            continue
    ...
    def _process_tool_calls(
        ...
        batch_auth_list = list(batch_authorized_symbols)
        def flush_authorized_segment() -> None:
            ...
            if len(authorized_segment) == 1:
                self._execute_single(
                    ...
                    react_trace,
                    iteration,
                    batch_authorized_symbols=batch_auth_list,
                )
            else:
                self._batch_execute(
                    authorized_segment,
    ...
    def _batch_execute(
        ...
        iteration: int,
        *,
        batch_authorized_symbols: list[str] | None = None,
    ) -> None:
        """Execute tools with read/write batching.
        ...
        Args:
            ...
            react_trace: React trace list.
            iteration: Current iteration.
            batch_authorized_symbols: Snapshot of authorized symbols.
        """
    ...
    def _is_tool_readonly(self, tool_name: str) -> bool:
        """Trả về liệu tool có được biết đến là không gây tác dụng phụ (side-effect) hay không."""
    ...
    def _finalize_tool_result(
        ...
        *,
        ...
        Args:
            ...
            batch_authorized_symbols: Snapshot of authorized symbols from authorization phase.
        """
        if update_memory:
        ...
        if isinstance(canonical_obj, (dict, list)):
            view_str = json.dumps(canonical_obj, ensure_ascii=False)[:TOOL_RESULT_LIMIT]
        else:
            view_str = canonical_str[:TOOL_RESULT_LIMIT]

        messages.append(context.format_tool_result(tc.id, tc.name, view_str))

        # Trace writer receives single redacted canonical payload
        trace_result = canonical_str
    ...
    def _auto_compact(
        ...
        Upgrades over the original:
          ...
          - Emergency mode: aggressive tail trimming + fallback summary on TPM/413 errors.
        ...
        Args:
            ...
            emergency: Set True when compressing in response to payload/TPM limits.
        """
        del run_dir
        ...
        # In emergency mode (TPM/413 limits), strictly limit tail to 3,000 tokens.
        effective_tail_budget = (
            3_000 if emergency
            else min(TAIL_TOKEN_BUDGET, max(2_000, int(_token_threshold() * 0.5)))
        )
        accumulated = 0
        cut_idx = len(body)
        for i in range(len(body) - 1, -1, -1):
            ...
            if accumulated + msg_tokens > effective_tail_budget:
                cut_idx = i + 1
                break
        ...
        raw_conv_text = json.dumps(head, default=str, ensure_ascii=False)
        max_conv_chars = 12000 if emergency else 80000
        summary_input_truncated = len(raw_conv_text) > max_conv_chars
        conv_text = raw_conv_text[:max_conv_chars]
        ...
        try:
            ...
            logger.warning("LLM call for auto_compact failed (%s), using structural fallback summary", exc)
            summary = (
                f"[Compressed summary (emergency fallback): previous history of {len(head)} turns "
                f"was compressed to fit payload/TPM limits. Memory state preserved below.]"
            )
    ...
    def _emit(self, event_type: str, data: Dict[str, Any]) -> None:
        """Phát sự kiện qua hàm gọi lại (callback)."""
    ...
    def _update_memory(self, tool_name: str) -> None:
        """Cập nhật bộ đếm bộ nhớ làm việc sau khi thực thi tool."""
