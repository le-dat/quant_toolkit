"""Công cụ remember: Thao tác lưu trữ ký ức liên phiên do LLM khởi xướng (save / recall / forget / reinforce)."""
...
from pseud.memory.lifecycle import MemoryLifecycle
from pseud.memory.persistent import PersistentMemory


class RememberTool(BaseTool):
    """Lưu, truy xuất hoặc xóa ký ức liên phiên làm việc.

    Ký ức được lưu bền vững vào <gốc runtime>/memory/ và tồn tại qua các phiên làm việc.
    """
    ...
    def __init__(self, memory: PersistentMemory | None = None) -> None:
        """Khởi tạo RememberTool.

        Args:
            memory: Instance PersistentMemory (tự động tạo nếu bỏ qua).
        """
        self._memory = memory or PersistentMemory()
        self._lifecycle = MemoryLifecycle(self._memory)

    def execute(self, **kwargs: Any) -> str:
        """Thực thi một hành động ghi nhớ.

        Args:
            **kwargs: Bắt buộc chứa action; các tham số khác tùy thuộc vào action.
    ...
    def _recall(self, kwargs: dict) -> str:
        ...
        # Theo dõi truy cập cho từng mục được gọi lại để sự suy giảm độ quan trọng (importance decay)
        # phản ánh đúng mẫu sử dụng thực tế.
        for e in entries:
