"""Hệ thống đa agent Swarm — lưu trữ trạng thái lượt chạy.

Lưu trữ dựa trên tệp cho SwarmRun. Cấu trúc thư mục:
    .swarm/runs/{run_id}/
    ├── run.json         # Trạng thái SwarmRun (ghi nguyên tử)
    ├── events.jsonl     # nhật ký sự kiện chỉ ghi nối tiếp (append-only)
    ├── tasks/           # tệp trạng thái tác vụ
    ├── inboxes/         # hòm thư nhắn agent
    └── artifacts/       # sản phẩm đầu ra của agent
"""
...
from pseud.swarm.models import SwarmEvent, SwarmRun
...
def _last_event_timestamp(events_file: Path) -> datetime | None:
    """Trả về dấu thời gian của dòng cuối cùng trong ``events.jsonl``.

    Sử dụng bởi bộ dọn dẹp lượt chạy bị treo (stale-run reaper). Trả về ``None`` khi tệp bị thiếu,
    rỗng, hoặc dòng cuối không thể phân tích cú pháp.
    """
...
def swarm_runs_root() -> Path:
    """Nguồn sự thật duy nhất về vị trí lưu trữ bền vững các lượt chạy swarm."""
    from pseud.config.paths import get_swarm_runs_dir
...
def _is_transient_windows_error(exc: OSError) -> bool:
    """Trả về True đối với lỗi tranh chấp truy cập/chia sẻ trên Windows khi os.replace."""
    return getattr(exc, "winerror", None) in _TRANSIENT_WINERRORS


def _replace_with_retry(tmp: Path, target: Path) -> None:
    """Hàm ``os.replace`` với cơ chế thử lại khi gặp xung đột truy cập trên Windows."""
...
class SwarmStore:
    """Lưu trữ bền vững dựa trên tệp cho SwarmRun.

    Mỗi lượt chạy được lưu dưới thư mục base_dir/{run_id}/.

    Thuộc tính:
        base_dir: Thư mục gốc lưu trữ.
    """
    ...
    def __init__(self, base_dir: Path) -> None:
        ...
        Args:
            base_dir: Đường dẫn thư mục gốc lưu trữ.
        """
        self.base_dir = base_dir
        self._write_lock = threading.Lock()

    def run_dir(self, run_id: str) -> Path:
        """Trả về đường dẫn thư mục cho một lượt chạy cụ thể.

        Args:
            run_id: Định danh lượt chạy.

        Returns:
            Đường dẫn Path tới thư mục lượt chạy.

        Raises:
            ValueError: Nếu run_id rỗng, là đường dẫn tuyệt đối, hoặc không hợp lệ.
        """
    ...
    def create_run(self, run: SwarmRun) -> Path:
        """Tạo cấu trúc thư mục cho lượt chạy mới và ghi trạng thái ban đầu.

        Args:
            run: Instance SwarmRun.

        Returns:
            Đường dẫn Path tới thư mục lượt chạy vừa tạo.

        Raises:
            FileExistsError: Nếu thư mục lượt chạy đã tồn tại.
        """
    ...
    def load_run(self, run_id: str) -> SwarmRun | None:
        """Nạp trạng thái cho một lượt chạy cụ thể.

        Args:
            run_id: Định danh lượt chạy.

        Returns:
            Instance SwarmRun, hoặc None nếu không tìm thấy.
        """
    ...
    def update_run(self, run: SwarmRun) -> None:
        """Cập nhật nguyên tử trạng thái lượt chạy.

        Args:
            run: Instance SwarmRun đã cập nhật.

        Raises:
            FileNotFoundError: Nếu thư mục lượt chạy không tồn tại.
        """
        rd = self.run_dir(run.id)
        if not rd.exists():
    ...
    def list_runs(self, limit: int = 50) -> list[SwarmRun]:
        """Liệt kê tất cả các lượt chạy sắp xếp theo thời gian tạo giảm dần.

        Args:
            limit: Số lượng lượt chạy tối đa trả về.

        Returns:
            Danh sách các instance SwarmRun.
        """
    ...
    def append_event(self, run_id: str, event: SwarmEvent) -> None:
        """Ghi nối tiếp một sự kiện vào events.jsonl.

        Args:
            run_id: Định danh lượt chạy.
            event: Sự kiện cần thêm.
    ...
    def read_events(self, run_id: str, after_index: int = 0) -> list[SwarmEvent]:
        """Đọc nhật ký sự kiện với offset tùy chọn cho SSE streaming.

        Args:
            run_id: Định danh lượt chạy.
            after_index: Bỏ qua N sự kiện đầu tiên.

        Returns:
            Danh sách các instance SwarmEvent.
        """
    ...
    def hydrate_run(self, run: SwarmRun) -> SwarmRun:
        """Trả về một bản sao của ``run`` với các tệp ``tasks/*.json`` mới nhất được hợp nhất vào."""
        from pseud.swarm.task_store import TaskStore
    ...
    def is_run_stale(self, run: SwarmRun, *, now: datetime | None = None) -> bool:
        ...
        """
        from pseud.swarm.models import RunStatus
    ...
    def reconcile_run(self, run: SwarmRun, *, write: bool = True) -> SwarmRun:
        ...
        """
        from pseud.swarm.models import RunStatus, TaskStatus
    ...
    def _reap_stale(self, run: SwarmRun, *, now: datetime) -> SwarmRun:
        ...
        from pseud.swarm.models import TaskStatus
